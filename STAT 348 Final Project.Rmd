...
```{r}
Logits <- glm(GRADUATED ~ IN_STATE+INTERNATIONAL+GENDER+American_Indian_or_Native_Alaskan+Asian+Black_or_African_American+Hispanic+Native_Hawaiian_or_Pacific_Islander+White+FGEN+LOAN+PLU_SCHOLARSHIP+EFC+EFC_trunc+FIRST_YEAR_CREDITS+DFWIS, data=SFA_Clean, family="binomial"(link="logit"))
...
ggplot(Coeff, aes(x = reorder(row.names(Coeff), exp(Logits.coefficients)), y = exp(Logits.coefficients), fill = row.names(Coeff)))+
   ...
   coord_flip()+
   labs(title = "First year actors associated with graduation",
        subtitle = "Removed high VIF variables",
        y = "Odds Ratios", 
        x =" ")+
  theme_test()+
  guides(fill="none")
...
```{r}
Logits <- glm(GRADUATED ~ IN_STATE+INTERNATIONAL+GENDER+American_Indian_or_Native_Alaskan+Asian+Black_or_African_American+Hispanic+Native_Hawaiian_or_Pacific_Islander+White+FGEN+PELL+LOAN+PLU_SCHOLARSHIP+EFC+EFC_trunc+FIRST_YEAR_CREDITS+DFWIS, data=SFA_Clean, family="binomial"(link="logit"))
 
summary(Logits)

step(Logits, direction = "both")

vif(Logits)
```
```{r}
Logits_Recc <- glm(formula = GRADUATED ~ IN_STATE + INTERNATIONAL + GENDER + 
    American_Indian_or_Native_Alaskan + Asian + Black_or_African_American + 
    Hispanic + Native_Hawaiian_or_Pacific_Islander + White + 
    FGEN + PELL + LOAN + PLU_SCHOLARSHIP + EFC + EFC_trunc + 
    FIRST_YEAR_CREDITS + DFWIS, family = binomial(link = "logit"), 
    data = SFA_Clean)
...
ggplot(Coeff, aes(x = reorder(row.names(Coeff), exp(Logits_Recc.coefficients)), 
   ...
   geom_hline(yintercept = 1, linetype = "dotted")+
   coord_flip()+
   labs(title = "First year factors associated with graduation",
        subtitle = "Forward Step Recommended Factors Using LRT. AIC = 6714",
        y = "Odds Ratios", 
        x =" ")+
  theme_test()+
  guides(fill="none")
...
Logits_Recc <- glm(formula = GRADUATED ~ IN_STATE + Asian + Native_Hawaiian_or_Pacific_Islander + 
    White + FGEN + PLU_SCHOLARSHIP + EFC_trunc + FIRST_YEAR_CREDITS + 
    DFWIS, family = binomial(link = "logit"), data = SFA_Clean)
...
ggplot(Coeff, aes(x = reorder(row.names(Coeff), exp(Logits_Recc.coefficients)), 
   ...
   coord_flip()+
   labs(title = "First year factors associated with graduation",
        subtitle = "Both Step Recommended Factors Using LRT. AIC = 6706",
        y = "Odds Ratios", 
        x =" ")+
  theme_test()+
  guides(fill="none")
...
ggplot(Coeff, aes(x = reorder(row.names(Coeff), exp(Logits_Recc.coefficients)), 
   ...
   coord_flip()+
   labs(title = "First year factors associated with graduation",
        subtitle = "Backward Step Recommended Factors Using LRT. AIC = 6706",
        y = "Odds Ratios", 
        x =" ")+
  theme_test()+
  guides(fill="none")
...
Note: Using LRT, the step() function takes all the variables and essentially whittles down to a list of recommended variables that maximizes the results of the LRT. The inputs I decided to provide were based on variables with low VIF from the first logistic regression model. This does not mean that the variables in the LRT recommended model are more important, it means that these added the most value to the model. 

To do next: We need to figure out how to show significance values and standard error for the odds ratios plot(s) that we choose to report. Additionally, we need to figure out which metric in the output reports model accuracy
